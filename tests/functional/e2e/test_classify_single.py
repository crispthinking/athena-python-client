...
from tests.functional.e2e.testcases.parser import load_test_cases, AthenaTestCase
...
async def test_classify_single(
    athena_options: AthenaOptions, credential_helper: CredentialHelper, test_case: AthenaTestCase
) -> None:
    ...
    channel = await create_channel_with_credentials(athena_options.host, credential_helper)
    ...
    async with AthenaClient(channel, athena_options) as client:
        ...
        ), f"Expected output to contain labels: {test_case.expected_output.keys() - actual_output.keys()}"

        max_diff = max(
            abs(test_case.expected_output[label] - actual_output[label]) for label in test_case.expected_output.keys()
        )
        assert (
            max_diff < FP_ERROR_TOLERANCE
        ), f"Output weights differ from expected by more than {FP_ERROR_TOLERANCE}: expected={test_case.expected_output}, actual={actual_output}"
